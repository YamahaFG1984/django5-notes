...
class CouponTests extends IntegrationTest {
    ...
    void setUp() throws Exception {
        ...
        mvc.perform(post("/cart/add/{id}/", greenTea).session(session).with(csrf()).param("quantity", "3"));
        mvc.perform(post("/cart/add/{id}/", redTea).session(session).with(csrf()).param("quantity", "1"));
    }

    private void apply(String code) throws Exception {
        mvc.perform(post("/coupons/apply/").session(session).with(csrf()).param("code", code))
    ...
    void expiredOrInactiveCouponsAreRejected() throws Exception {
        ...
        mvc.perform(get("/cart/").session(session)).andExpect(content().string(not(containsString("coupon ("))));
        apply("OFF");
        mvc.perform(get("/cart/").session(session)).andExpect(content().string(not(containsString("coupon ("))));
    }
