...
class ShopAndCartTests extends IntegrationTest {
    ...
    void catalogListsAvailableProductsByCategory() throws Exception {
        ...
        mvc.perform(get("/no-such-category/")).andExpect(status().isNotFound());
        mvc.perform(get(greenTea.getAbsoluteUrl())).andExpect(status().isNotFound());
        mvc.perform(get(redTea.getAbsoluteUrl()))
    ...
    void cartAddUpdateRemove() throws Exception {
        mvc.perform(post("/cart/add/{id}/", greenTea.getId()).session(session).with(csrf())
                        .param("quantity", "2").param("override", "false"))
                .andExpect(redirectedUrl("/cart/"));
        mvc.perform(post("/cart/add/{id}/", greenTea.getId()).session(session).with(csrf())
                        .param("quantity", "1").param("override", "false"));
        mvc.perform(post("/cart/add/{id}/", redTea.getId()).session(session).with(csrf())
                        .param("quantity", "1").param("override", "false"));
        ...
        mvc.perform(post("/cart/remove/{id}/", redTea.getId()).session(session).with(csrf()));
        ...
        // 另一个会话看不到这个购物车
        mvc.perform(get("/")).andExpect(content().string(containsString("Your cart is empty.")));
    }

    @Test
    void invalidQuantityIsIgnored() throws Exception {
        mvc.perform(post("/cart/add/{id}/", greenTea.getId()).session(session).with(csrf()).param("quantity", "99"));
        mvc.perform(get("/cart/").session(session))
    ...
    void checkoutCreatesOrderWithItemsAndClearsCart() throws Exception {
        mvc.perform(get("/orders/create/").session(session)).andExpect(redirectedUrl("/cart/"));

        mvc.perform(post("/cart/add/{id}/", redTea.getId()).session(session).with(csrf()).param("quantity", "2"));
        mvc.perform(post("/orders/create/").session(session).with(csrf()).param("firstName", ""))
        ...
        mvc.perform(post("/orders/create/").session(session).with(csrf())
                ...
                .andExpect(content().string(containsString("Your order number is")));
        ...
        mvc.perform(get("/").session(session)).andExpect(content().string(containsString("Your cart is empty.")));
    }
}
