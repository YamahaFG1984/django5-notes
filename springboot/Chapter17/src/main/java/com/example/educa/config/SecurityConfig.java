...
public class SecurityConfig {
    ...
    SecurityFilterChain apiFilterChain(HttpSecurity http, EducaProperties properties) throws Exception {
        requireHttps(http, properties);
    ...
    SecurityFilterChain securityFilterChain(HttpSecurity http, SecurityContextRepository contextRepository,
                                            EducaProperties properties) throws Exception {
        requireHttps(http, properties);
        http
                .authorizeHttpRequests(auth -> auth
    ...
    /**
     * ≈ SECURE_SSL_REDIRECT = True：HTTP 请求重定向到 HTTPS（生产环境开启）。
     * 在 Nginx 后面时，应用看到的协议来自 X-Forwarded-Proto（server.forward-headers-strategy=native）。
     * /actuator/** 不重定向：容器内部的健康检查直接用 http://localhost:8080 访问。
     * HTTPS 请求上 Spring Security 默认还会加 Strict-Transport-Security 头（≈ SECURE_HSTS_SECONDS）。
     */
    private static void requireHttps(HttpSecurity http, EducaProperties properties) throws Exception {
        if (properties.requireHttps()) {
            http.redirectToHttps(https -> https.requestMatchers(
                    new NegatedRequestMatcher(PathPatternRequestMatcher.withDefaults().matcher("/actuator/**"))));
        }
    }
