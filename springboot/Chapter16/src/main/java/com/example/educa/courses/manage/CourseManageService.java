...
import com.example.educa.courses.catalog.CacheNames;
import com.example.educa.courses.catalog.EvictCatalog;

/**
 * 讲师管理自己课程的业务逻辑。所有查询都带上 ownerId —— 这就是书中 OwnerMixin 和
 * get_object_or_404(..., owner=request.user) 做的事：别人的课程一律当作“不存在”（404）。
 * <p>
 * 第 14 章：写操作顺带清除公开目录的缓存（@EvictCatalog、@CacheEvict），学生总能看到最新内容。
 */
@Service
@Transactional
public class CourseManageService {
    ...
    @EvictCatalog
    @CacheEvict(cacheNames = CacheNames.MODULE_CONTENTS, allEntries = true)
     ...
     */
    @Caching(evict = {
            @CacheEvict(cacheNames = CacheNames.COURSES, allEntries = true),          // 模块数变了
            @CacheEvict(cacheNames = CacheNames.MODULE_CONTENTS, allEntries = true)   // 可能删除了模块
    })
     ...
     */
    @CacheEvict(cacheNames = CacheNames.MODULE_CONTENTS, key = "#moduleId")
    ...
    public Long deleteContent(Long contentId, Long ownerId) {
        ...
        // 模块 id 要查出内容之后才知道，注解里写不出 key，这里用编程方式清除
        Cache cache = cacheManager.getCache(CacheNames.MODULE_CONTENTS);
        if (cache != null) {
