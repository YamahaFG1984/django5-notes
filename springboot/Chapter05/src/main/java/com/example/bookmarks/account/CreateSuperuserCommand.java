 ...
 * <pre>
 * java -jar target/bookmarks-1.0.0.jar --spring.main.web-application-type=none \
...
public class CreateSuperuserCommand implements ApplicationRunner {
    ...
    public CreateSuperuserCommand(UserRepository users, ProfileRepository profiles, PasswordEncoder passwordEncoder) {
        this.users = users;
    ...
    public void run(ApplicationArguments args) {
        ...
        users.save(user);
        profiles.save(new Profile(user));   // 原书的超级用户没有 Profile，访问“编辑资料”会出错；这里一并创建
