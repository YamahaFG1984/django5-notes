...
class ShopAdminTests extends IntegrationTest {
    ...
    void listEditableSavesPriceAndAvailabilityOfManyRows() throws Exception {
        List<Product> all = ProductRepository.sortedByName(products.findAllBy());   // Espresso, Green tea, Red tea, Tea powder
    ...
    void addProductWithImage() throws Exception {
        Long teaId = categories.findByTranslatedSlug("en", "tea").orElseThrow().getId();
        mvc.perform(multipart("/admin/shop/product/add/").file(new MockMultipartFile("image", "oolong.png", "image/png", png()))
                        .param("categoryId", teaId.toString())
                        .param("translations[en].name", "Oolong").param("translations[en].slug", "oolong")
                        .param("translations[es].name", "Oolong ES").param("translations[es].slug", "oolong-es")
        ...
        assertThat(oolong.translation("es").getSlug()).isEqualTo("oolong-es");

        // 默认语言必填；另一种语言只填了一半也会报错
        mvc.perform(multipart("/admin/shop/product/add/")
                        .param("categoryId", teaId.toString()).param("price", "1.00")
                        .param("translations[es].name", "Solo nombre")
                        .with(staff).with(csrf()))
                .andExpect(org.springframework.test.web.servlet.result.MockMvcResultMatchers.model()
                        .attributeHasFieldErrors("form", "translations[en].name", "translations[en].slug", "translations[es].slug"));
    }
