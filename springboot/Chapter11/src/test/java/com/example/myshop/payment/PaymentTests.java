...
class PaymentTests extends IntegrationTest {
    ...
    void processShowsSummaryAndRedirectsToStripeWith303() throws Exception {
        mvc.perform(get("/en/payment/process/")).andExpect(status().isNotFound());   // 会话里没有订单
        mvc.perform(get("/en/payment/process/").session(session))
                .andExpect(content().string(containsString("$60.00")));

        when(gateway.createCheckoutSession(any(), eq("http://localhost/en/payment/completed/"), eq("http://localhost/en/payment/canceled/")))
                .thenReturn("https://checkout.stripe.com/c/pay/cs_test_1");
        mvc.perform(post("/en/payment/process/").session(session).with(csrf()))
