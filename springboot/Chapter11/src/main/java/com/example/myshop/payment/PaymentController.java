...
public class PaymentController {
     ...
     */
    @PostMapping("/process/")
    public ResponseEntity<Void> process(HttpSession session, HttpServletResponse response) {
        Order order = currentOrder(session);
        // 上下文路径里带着语言前缀（/es）；encodeURL 再把路径片段翻译成当前语言（/es/pago/completado/）
        String base = ServletUriComponentsBuilder.fromCurrentContextPath().toUriString();
        String url = stripe.createCheckoutSession(order,
                response.encodeURL(base + "/payment/completed/"), response.encodeURL(base + "/payment/canceled/"));
