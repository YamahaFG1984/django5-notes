    ...
    public String createCheckoutSession(Order order, String successUrl, String cancelUrl) {
        ...
        try {
            if (order.getCoupon() != null) {
                // 在 Stripe 上即时创建一个一次性的百分比优惠券（≈ stripe.Coupon.create(..., duration='once')）
                Coupon stripeCoupon = stripe.v1().coupons().create(CouponCreateParams.builder()
                        .setName(order.getCoupon().getCode())
                        .setPercentOff(BigDecimal.valueOf(order.getDiscount()))
                        .setDuration(CouponCreateParams.Duration.ONCE)
                        .build());
                params.addDiscount(SessionCreateParams.Discount.builder().setCoupon(stripeCoupon.getId()).build());
            }
