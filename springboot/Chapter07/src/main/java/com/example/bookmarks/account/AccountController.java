...
import com.example.bookmarks.images.BookmarkletLauncher;
...
public class AccountController {
    ...
    private final BookmarkletLauncher bookmarklet;
    private final ActionService actions;

    public AccountController(AccountService accounts, UserRepository users, ImageService images,
                             BookmarkletLauncher bookmarklet, ActionService actions) {
    ...
    @GetMapping("/")
    public String dashboard(@AuthenticationPrincipal CurrentUser me, Model model) {
        model.addAttribute("section", "dashboard");
        model.addAttribute("totalImagesCreated", images.countCreatedBy(me.id()));
        model.addAttribute("bookmarklet", bookmarklet.href());
        model.addAttribute("actions", actions.feedFor(me.id(), 10));
