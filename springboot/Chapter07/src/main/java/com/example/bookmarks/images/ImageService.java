...
public class ImageService {
    ...
    public ImageService(ImageRepository images, UserRepository users, ProfileRepository profiles,
                        ImageDownloader downloader, MediaStorage media, ActionService actions,
    ...
    public Image create(Long userId, ImageCreateForm form) {
        ...
        Image image = images.save(new Image(users.getReferenceById(userId), form.getTitle(), form.getUrl(),
                path, form.getDescription()));
        actions.create(userId, "bookmarked image", image);
        return image;
    }
    ...
    public boolean like(Long imageId, Long userId, boolean like) {
        return images.findById(imageId).map(image -> {
            ...
            if (like && !alreadyLiked) {          // User 没有重写 equals，按 id 判断，避免重复插入中间表
                ...
                actions.create(userId, "likes", image);
            } else if (!like) {
                image.getUsersLike().removeIf(u -> u.getId().equals(userId));
            }
            events.publishEvent(new ImageLikesChangedEvent(imageId));   // ≈ Django 自动发出的 m2m_changed 信号
            return true;
        }).orElse(false);
    }

    /** 按 Redis 排行给出的 id 顺序返回图片（数据库 IN 查询不保证顺序，要自己排） */
    @Transactional(readOnly = true)
    public List<Image> inOrder(List<Long> ids) {
        Map<Long, Image> byId = images.findAllById(ids).stream()
                .collect(Collectors.toMap(Image::getId, image -> image));
        return ids.stream().filter(byId::containsKey).map(byId::get).toList();
    }

    @Transactional(readOnly = true)
    public List<Image> createdBy(Long userId) {
        return images.findByUserIdOrderByCreatedDesc(userId);
    }
